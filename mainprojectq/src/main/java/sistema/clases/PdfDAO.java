...
import sistema.clases.Conectar;
...
public class PdfDAO {
    ...
    public ArrayList<PdfVO> Listar_PdfVO() {
        ...
        try {
            ...
            while (rs.next()) {
                PdfVO vo = new PdfVO();
                vo.setIddieta(rs.getInt(1));
                vo.setNombrepdf(rs.getString(2));
                vo.setIdcoach(rs.getInt(3));
    ...
    public void Agregar_PdfVO(PdfVO vo) {
        Conectar conec = new Conectar();
        String sql = "INSERT INTO dieta (iddieta, nombre, idcoach, archivo) VALUES(?, ?, ?, ?);";
        PreparedStatement ps = null;
        try {
            ps = conec.getConnection().prepareStatement(sql);
            ps.setInt(1, vo.getIddieta());
            ps.setString(2, vo.getNombrepdf());
            ps.setInt(3, vo.getIdcoach());
    ...
    public void Modificar_PdfVO(PdfVO vo) {
        Conectar conec = new Conectar();
        String sql = "UPDATE dieta SET nombre = ?, idcoach = ?, archivo =? WHERE iddieta = ?;";
        PreparedStatement ps = null;
        try {
            ps = conec.getConnection().prepareStatement(sql);
            ps.setString(1, vo.getNombrepdf());
            ps.setInt(2, vo.getIdcoach());
            ps.setBytes(3, vo.getArchivopdf());
            ps.setInt(4, vo.getIddieta());
            ps.executeUpdate();
        } catch (SQLException ex) {
            System.out.println(ex.getMessage());
        } catch (Exception ex) {
    ...
    public void Modificar_PdfVO2(PdfVO vo) {
        Conectar conec = new Conectar();
        String sql = "UPDATE dieta SET nombre = ? WHERE iddieta = ?;";
        PreparedStatement ps = null;
        try {
            ps = conec.getConnection().prepareStatement(sql);
            ps.setString(1, vo.getNombrepdf());
            ps.setInt(2, vo.getIddieta());
            ps.executeUpdate();
        } catch (SQLException ex) {
            System.out.println(ex.getMessage());
        } catch (Exception ex) {
    ...
    public void ejecutar_archivoPDF(int id) {
        ...
        try {
            ps = cn.getConnection().prepareStatement("SELECT archivo FROM dieta WHERE iddieta = ?;");
            ps.setInt(1, id);
            rs = ps.executeQuery();
            while (rs.next()) {
                b = rs.getBytes(1);
            ...
            System.out.println("Error al abrir archivo WORD " + ex.getMessage());
        }
    }
